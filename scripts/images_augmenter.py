...
def parse_arguments():
    ...
    parser.add_argument(
        "--save_path", default=r"", help="Folder where augmented images will be saved"
    )
    parser.add_argument(
        "--total_images",
        type=int,
        default=100,
        help="Number of augmentation images required",
    )

    parser.add_argument(
        "--rotation_limit", type=int, default=90, help="Max angle of rotation"
    )
    parser.add_argument(
        "--rotation_thresh",
        type=float,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[0.5, 1.0],
        help="Transparency range in percent",
    )
    ...
    def __call__(self, image: np.ndarray, background_image: np.ndarray) -> np.ndarray:
        ...
        for transform in self.augmentation:
            if transform.name == "rotation" and random.random() < self.rotation_thresh:
                ...
                transform.name == "deformation"
                and random.random() < self.deformation_thresh
            ):
                continue
        ...
        try:
            x = random.randint(0, background_image.shape[1] - int(image.shape[1] * 0.8))
            y = random.randint(0, background_image.shape[0] - int(image.shape[0] * 0.8))
            image = self.overlay(background_image, image, x, y)
        except Exception as e:
            print(
                f"Failed while generating location for the augmented image. Error: {e}"
            )
            return
    ...
    def overlay(
        ...
        if random.random() > self.transp_thresh:
            transparency_factor = float(
                random.randint(int(self.transp_min * 100), int(self.transp_max * 100))
                / 100
            )
            assert (
                0 <= transparency_factor <= 1
            ), "Wrong transparency factor. Expected [0.0, 1.0]"
...
class Rotation:
    ...
    def __init__(self, rotation_limit: int):
        self.name = "rotation"
        assert (
            0 <= rotation_limit <= 180
        ), "Wrong rotation range value. Expected: [0, 180]"
...
class Resize:
    ...
    def __call__(self, image: np.ndarray, background_size: List[int]) -> np.ndarray:
        ...
        resize_factor = (
            random.randint(int(self.min_allowed * 100), int(self.max_allowed * 100))
            / 100
        )
        ...
        if image.shape[0] > image.shape[1]:
            ...
            new_image_width = (
                background_width * resize_factor
            )  # size of rgba image relatively to the background image
            aspect_ratio_factor = new_image_width / image_width
            new_image_height = image_height * aspect_ratio_factor
        resized_image = cv2.resize(
            image, dsize=(int(new_image_width), int(new_image_height))
        )
...
class Deformation:
    ...
    def __init__(self, deformation_limit: float = 0.2):
        ...
        assert (
            0 <= deformation_limit < 1.0
        ), "Wrong deformation limit range. Expected 0 - 99%"
        self.limit = deformation_limit

    def __call__(self, image: np.ndarray, background_size: List[int]) -> np.ndarray:
        # Select random: deformation factor
        deformation_factor = (
            random.randrange(int(100 - self.limit * 100), int(100 + self.limit * 100))
            / 100.0
        )
        ...
        image_height, image_width = image.shape[:2]
        new_size = (
            (image_width, int(image_height * deformation_factor))
            if axis == 0
            else (int(image_width * deformation_factor), image_height)
        )
...
def augment_images(
    ...
    while True:
        ...
        if augmented_image is not None:
            try:
                cv2.imwrite(
                    os.path.join(save_path, str(total_augmented) + ".jpg"),
                    augmented_image,
                )
            except Exception as e:
...
def main():
    ...
    assert (
        0 <= args.rotation_limit <= 180
    ), "Wrong argument rotation limit. Expected [0, 180]"
    assert (
        0.0 <= args.deformation_limit < 1
    ), "Wrong argument deformation limit. Expected [0, 1)"
    ...
    assert (
        0.0 <= args.rotation_thresh <= 1.0
    ), "Wrong rotation threshold. Expected [0, 100]"
    assert (
        0.0 <= args.deform_thresh <= 1.0
    ), "Wrong deformation threshold. Expected [0, 100]"
    transparency_range = [float(e) for e in args.transparency_range]
    assert all(
        0.0 <= e <= 1.0 for e in transparency_range
    ), "Wrong value of transparency range. Expected [0, 1]"
    assert (
        0.0 <= args.transparency_thresh <= 1.0
    ), "Wrong transprency thresh. Expected [0, 100]"
    ...
    if not_rgba:
        print(
            "Detected some not RGBA images in the folder with images to augment. Attempting to transform them to RGBA"
        )
